...
export const Ruler = () => {
  ...
  // Margins are page setup, not prose, so they live in the document's `meta`
  // map — shared and undo-able alongside the text, exactly like the old
  // Liveblocks Storage entry they replace.
  const { doc } = useCollaboration();
  const [{ leftMargin, rightMargin }, setMargins] = useState(() => getMargins(doc));
  ...
  const setLeftMargin = useCallback(
    (position: number) => setMargin(doc, "leftMargin", position),
    [doc],
  );
  const setRightMargin = useCallback(
    (position: number) => setMargin(doc, "rightMargin", position),
    [doc],
  );
